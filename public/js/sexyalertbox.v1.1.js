  ...
  preloadImages: function() {
	...
	display: function(option){
      ...
      if(Browser.Engine.trident4)
        $$('select', 'object', 'embed').each(function(node){ node.style.visibility = 'hidden' });
			...
			this.Transition = new Fx.Tween2(this.Overlay,
				{
          property: 'opacity',
	...
	*/
	replaceBox: function() {
      ...
      scrollito = window.getScroll();
			...
			this.MoveBox = new Fx.Morph2(this.Box, {
	...
	*/	
	messageBox: function(type, message, properties, input) {
      ...
      else if(type == 'select')
      {
          ...
          this.PromptInput = new Element('select', {
            'id': 'BoxPromptInput'
          });

          for (i=0;i<input.length;i++) {
             var y = document.createElement('option');
             y.value = i+1; // otherwise the first option won't work
             y.text = input[i];
             if (i == 0) { y.selected = true; }
             this.PromptInput.add(y,null);
          }

          this.PromptBtnOk.addEvent('click', function() {
            this.options.onReturn = this.PromptInput.options[this.PromptInput.selectedIndex].value;
            this.display(0);
          }.bind(this));
	...
	*/	
	...
	Argument:
		properties - see Options in messageBox
	*/	
	select: function(message, input, properties){
		this.messageBox('select', message, properties, input);
	}
